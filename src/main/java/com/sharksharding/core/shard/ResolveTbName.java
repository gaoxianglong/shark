...
public class ResolveTbName {
	 ...
	 * @return tbName 数据库表名
	 */
	public static String getTbName(String sql) {
	...
	public static String getNewTbName(int index, String tbName, String tbSuffix) {
		...
		if (indexlength < rightLength) {
			...
			newTbName = tbName + left + placeholder.toString() + index;
		} else {
			newTbName = tbName + left + index;
		}
		return newTbName;
	}
}